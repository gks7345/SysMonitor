...
SessionTargetSummary SummaryStore::parseTargetRow(
    duckdb::DataChunk* chunk, size_t row) {
    ...
    t.sampleCount = std::stoi(chunk->GetValue(12, row).ToString());
    return t;
}
...
void SummaryStore::flushSysSummary(const SessionSysSummary& s) {
    ...
    if (!chunk || chunk->size() == 0) {
        ...
        int    n = std::stoi(chunk->GetValue(8, 0).ToString());
...
// 프로세스 요약 저장
// -------------------------------------------------------
void SummaryStore::flushProcSummaries(
    ...
    for (const auto& p : procs) {
        ...
        if (!chunk || chunk->size() == 0) {
            ...
            double ma = safeStod(chunk->GetValue(2, 0).ToString());
            double mk = safeStod(chunk->GetValue(3, 0).ToString());
            int    n = std::stoi(chunk->GetValue(4, 0).ToString());
...
// 타겟 요약 저장
// -------------------------------------------------------
void SummaryStore::flushTargetSummaries(
    ...
    for (const auto& t : targets) {
        ...
        if (!chunk || chunk->size() == 0) {
            // 신규 삽입
            con.Query(
                "INSERT INTO target_summary VALUES ("
                "'" + t.date + "', "
                "'" + t.targetName + "', "
                "'" + t.exePath + "', " +
            ...
            int    n = std::stoi(chunk->GetValue(9, 0).ToString());
            ...
            con.Query(
                ...
                "totalRuntime = " + std::to_string(el + t.runTime) + ", "
                "sampleCount = " + std::to_string(total) + " "
                "WHERE date = '" + t.date + "' "
                "  AND targetName = '" + t.targetName + "'");
        }
...
SessionTargetSummary SummaryStore::getTargetLastSession(const std::string& name) {
    ...
    if (!r->HasError()) {
        auto chunk = r->Fetch();
        if (chunk && chunk->size() > 0) {
            auto result = parseTargetRow(chunk.get(), 0);
    ...
    if (!r2->HasError()) {
        ...
        if (chunk && chunk->size() > 0) {
            auto result = parseTargetRow(chunk.get(), 0);
            spdlog::info("SummaryStore: '{}' Session return Last day ({})", name, result.date);
            return result;
        }
    }
